...
**功能改进：**
- **为每个卡片类型设计独特的背景样式** - 5 种不同的渐变配色
- **视觉识别增强** - 通过颜色快速识别账户类型
- **设计细节优化** - 顶部渐变条、类型指示圆点、着色标签

**设计规范：**

| 类型 | 颜色方案 | 含义 | 效果 |
|------|--------|------|------|
| 🔐 **密码** | 深蓝 → 蓝 | 安全、信任、保护 | 专业感 |
| 🟠 **验证码** | 红 → 橙 | Google 品牌、实时 | 现代感 |
| 💜 **助记词** | 紫 → 浅紫 | 神秘、加密、重要 | 优雅感 |
| 💛 **银行卡** | 棕 → 金 | 财富、价值、信任 | 奢华感 |
| ❤️ **身份证** | 深红 → 红 | 官方、正式、权威 | 严肃感 |
...
| 类型 | 深色 | 浅色 | Hex 深 | Hex 浅 |
|------|------|------|--------|--------|
| 密码 | #1e3a8a | #3b82f6 | 深蓝 | 蓝 |
| 验证码 | #EA4335 | #FF9500 | Google红 | 橙 |
| 助记词 | #7c3aed | #a855f7 | 紫 | 浅紫 |
| 银行卡 | #92400e | #d97706 | 棕 | 金 |
| 身份证 | #7f1d1d | #dc2626 | 深红 | 红 |
...
- 🎨 **快速识别** - 一眼就能看出是哪种类型的账户
- 💡 **视觉美观** - 彩色渐变增强了界面的现代感
- 📊 **信息层级** - 通过颜色强化重要信息的视觉优先级
- 🧠 **心理关联** - 颜色与内容的关联提高理解速度
- 🔍 **快速扫描** - 用户可以快速扫描列表找到目标卡片
- 📱 **品牌统一** - Google 验证码使用官方品牌色

**文件变更：**
- ✅ 新建：`app/src/main/java/x/x/p455w0rd/ui/compose/CardBackgroundStyles.kt`
  - 定义每个类型的背景样式
  - 提供颜色获取函数
  - 支持未来的渐变方向扩展

- ✅ 更新：`app/src/main/java/x/x/p455w0rd/ui/compose/PasswordItemCard.kt`
  - 添加导入：`background`, `drawBehind`, `Brush`, `Color`
  - 在卡片顶部绘制 4dp 的渐变条
  - 添加类型指示圆点
  - 为类型标签应用对应的颜色
  - 提升卡片投影到 4dp

- ✅ 新建：`CARD_DESIGN_GUIDE.md`
  - 详细的设计规范文档
  - 颜色心理学解释
  - UI 显示效果示例
  - 尺寸规范表
